...
<?php include 'includes/topbar.php';?>
    ...
    <div class="table-container">
        ...
        individually script for webcam
        <script>
        // Initialize webcam variables
        const webcamElement = document.getElementById('video');
        const webcam = new Webcam(webcamElement, 'user', document.getElementById('video-container'));
        const webcamSelect = document.getElementById('webcam-select');
        const startWebcamButton = document.getElementById('startButton');
        const stopWebcamButton = document.getElementById('endAttendance');

        // Populate available webcams in the select dropdown
        navigator.mediaDevices.enumerateDevices()
            ...
            .catch(error => console.error('Unable to access cameras', error));

        // Update webcam based on selection
        webcamSelect.addEventListener('change', function() {
            if (webcamSelect.value) {
                webcam.stop();
                webcam.start(webcamSelect.value)
                    .then(result => {
        ...
        // Start Webcam Button
        startWebcamButton.addEventListener('startButton', () => {
            webcam.start()
                .then(result => {
        ...
        // Stop Webcam Button
        stopWebcamButton.addEventListener('endAttendance', () => {
            webcam.stop();
